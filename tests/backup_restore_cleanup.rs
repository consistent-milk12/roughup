...
fn overwrite_manifest_timestamp(repo: &Path, sess: &str, rfc3339: &str) {
    let manifest = repo.join(".rup/backups").join(sess).join("manifest.json");
...
fn test_restore_reports_conflict_on_dry_run() {
    ...
    assert!(out.diffs.as_ref().unwrap()[0].unified.contains("@@"));
...
fn test_restore_force_overwrites_and_backs_up_current() {
    ...
    assert!(out.backup_session_id.is_some());
...
fn test_restore_single_path_filter() {
    ...
    mgr.backup_file(Path::new("a.txt")).unwrap();
    mgr.backup_file(Path::new("b.txt")).unwrap();
    mgr.finalize(true).unwrap();
    ...
    assert!(out_a.diffs.unwrap()[0].unified.contains("@@"));
}
...
fn test_cleanup_by_age_and_keep_latest() {
    ...
    assert!(out.sessions_removed.contains(&s1));
    // Ensure s3 (latest) is not slated for removal.
    assert!(!out.sessions_removed.contains(&s3));
    ...
    assert!(out2.sessions_removed.contains(&s1));
    assert!(out2.sessions_removed.contains(&s2));
    assert!(!out2.sessions_removed.contains(&s3));
    // The directory for s3 must still exist.
    assert!(repo.join(".rup/backups").join(&s3).exists());
}
