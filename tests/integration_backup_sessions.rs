...
use roughup::core::apply_engine::{ApplyContext, ApplyEngine, InternalEngine, HybridEngine};
...
fn test_internal_engine_single_session_success() -> Result<()> {
    ...
    assert_eq!(report.engine_used, roughup::core::apply_engine::Engine::Internal);
...
fn test_auto_fallback_shared_session() -> Result<()> {
    ...
    match result {
        ...
        Ok(report) => {
            ...
            assert_eq!(report.engine_used, roughup::core::apply_engine::Engine::Auto);
            assert!(report.backup_session_id.is_some());
        }
    }
