...
use roughup::{
    cli::WhitespaceMode,
    core::{
        apply_engine::{ApplyContext, ApplyEngine, HybridEngine, InternalEngine},
        backup::BackupManager,
        edit::{EditOperation, EditSpec, FileBlock},
        git::GitOptions,
    },
};
...
fn test_auto_fallback_shared_session() -> Result<()>
{
    ...
    match result
    {
        Err(_) =>
        {
            ...
            // Internal has conflicts and finalizes session, preventing Git fallback with
            // backup
        }
        Ok(report) =>
        {
