...
//!
//! Notes:
//!   - We only query for functions and classes. Methods are determined by detecting a
//!     surrounding class_definition.
//!   - We always pass the same byte slice that Parser parsed.
//!   - We rely on tree_sitter::StreamingIterator for matches.
//!   - Docstrings support single/triple quotes and common prefixes (r, u, f, fr, rf).
//!     Dedent is applied for triple-quoted docs. Concatenated string docstrings are
//!     joined segment-wise.
//! ------------------------------------------------------------------
...
impl SymbolExtractor for PythonExtractor
{
    ...
    ) -> Result<Vec<Symbol>>
    {
        ...
        while let Some(m) = matches.next()
        {
            ...
            let Some(node) = picked
            else
            {
                continue;
            };
            let Some(name) = name_text
            else
            {
                continue;
            };
...
fn python_qualified_name_method(
    mut node: Node,
    bytes: &[u8],
    method_name: &str,
) -> String
...
mod tests
{
    ...
    #[test]
    fn python_extractor_is_deterministic() -> Result<()>
    {
        let src = r#"def a(): pass
def b(): pass"#;
        let ext = PythonExtractor::new()?;
        let mut s1 = ext.extract_symbols(src, std::path::Path::new("m.py"))?;
        let mut s2 = ext.extract_symbols(src, std::path::Path::new("m.py"))?;
...
"#;
        ...
        assert!(
            syms.iter()
                .any(|s| has(s, SymbolKind::Class, "MyClass"))
        );
