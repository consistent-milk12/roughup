...
fn capture_git_snapshot(repo_root: &Path) -> Result<GitSnapshot>
{
    ...
    Ok(GitSnapshot { commit, branch, dirty, staged })
}
...
fn acquire_lock(lock_path: &Path) -> Result<LockGuard>
{
        ...
        .open(lock_path)
    {
        Ok(mut file) =>
        {
            ...
            file.sync_all()
                .ok();
            Ok(LockGuard { path: lock_path.to_path_buf(), file })
        }
        Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists =>
        {
                ...
                && elapsed.as_secs() > 60
            {
                ...
                if fs::remove_file(lock_path).is_ok()
                {
                    ...
                    return Ok(LockGuard { path: lock_path.to_path_buf(), file });
                }
            }
...
mod tests
{
    ...
    fn basic_session_flow()
    {
        ...
        mgr.backup_file(Path::new("file.txt"))
            .unwrap();
        mgr.finalize(true)
            .unwrap();
    ...
    fn preserves_mirrored_tree()
    {
        ...
        mgr.backup_file(Path::new("src/core/x.rs"))
            .unwrap();
        mgr.finalize(true)
            .unwrap();

        let id = list_sessions(repo).unwrap()[0]
            .id
            .clone();
        let backed = repo
            .join(".rup/backups")
