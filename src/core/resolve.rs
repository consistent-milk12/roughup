...
use crate::{
    cli::{AppContext, ResolveArgs},
    core::{
        backup::BackupManager,
        conflict::{ConflictMarker, parse_conflicts},
    },
};
...
{
    let ours_empty = conflict
        .ours
        .trim()
        .is_empty();
    let theirs_empty = conflict
        .theirs
        .trim()
        .is_empty();
...
mod tests
{
    ...
    fn test_addition_only_resolution()
    {
        ...
        assert!(
            resolution
                .reason
                .contains("addition-only-ours-empty")
        );
        ...
        assert!(
            resolution
                .reason
                .contains("addition-only-theirs-empty")
        );
    }
    ...
    fn test_superset_resolution()
    {
        ...
        assert!(
            resolution
                .reason
                .contains("superset-ours-contains-theirs")
        );
    }
    ...
    fn test_disjoint_insertion_resolution()
    {
        ...
        assert!(
            resolution
                .reason
                .contains("disjoint-insertions")
        );

        // Verify ours comes before theirs
        let ours_pos = resolved
            .find("function_a")
            .unwrap();
        let theirs_pos = resolved
            .find("function_b")
            .unwrap();
        assert!(
            ours_pos < theirs_pos,
    ...
    fn test_syntax_validation()
    {
        ...
        assert!(
            resolution
                .reason
                .contains("Syntax validation failed")
        );
    }
    ...
    fn test_batch_resolution()
    {
        let conflicts = vec![
            ...
            make_test_conflict("complex", "different", None, 0.50), /* low confidence, no clear
                                                             * pattern */
        ];

        let conflict_refs: Vec<&ConflictMarker> = conflicts
            .iter()
            .collect();
...
) -> Result<()>
{
    ...
    for summary in &mut file_summaries
    {
        ...
        for (conflict, resolution) in file_conflicts
            .iter()
    ...
    if args.json
    {
        let has_unresolved = file_summaries
            .iter()
            .any(|s| s.interactive_required > 0);
...
)
{
    ...
    let total_conflicts: usize = summaries
        .iter()
        .map(|s| s.total_conflicts)
        .sum();
    let total_resolved: usize = summaries
        .iter()
        .map(|s| s.auto_resolved)
        .sum();
    let total_interactive: usize = summaries
        .iter()
        .map(|s| s.interactive_required)
        .sum();
