...
use crate::{
    ...
    core::{
        ...
        backup_ops::{
            CleanupRequest, ListRequest, RestoreRequest, SessionInfo, ShowRequest,
            cleanup_sessions, list_sessions_filtered, restore_session, show_session,
        },
        resolve::run as resolve_run,
    },
};
...
pub fn normalize_err_typed(e: anyhow::Error) -> (ApplyErr, i32)
{
    ...
    {
        return (ApplyErr::Conflicts { details: vec![msg.clone()] }, 2);
    }
...
impl EditEngine
{
    ...
    pub fn parse_edit_spec(
        &self,
        input: &str,
    ) -> Result<EditSpec, ParseError>
    {
            ...
            if line.starts_with("FILE:")
            {
                ...
                while i < lines.len()
                {
                    ...
                    match self.parse_operation(&lines, &mut i)?
                    {
                        ...
                        None =>
                        {
                            // parse_operation advanced i itself; if not, advance by one to avoid a
                            // stall
                            if i == before
                            {
                                i += 1;
    ...
    ) -> Result<Option<EditOperation>, ParseError>
    {
        ...
        *i += 1;
        ...
        Ok(Some(EditOperation::Insert { at_line, new_content }))
    }
    ...
    ) -> Result<Option<EditOperation>, ParseError>
    {
        ...
        *i += 1;
        ...
        Ok(Some(EditOperation::Delete { start_line, end_line }))
    }
    ...
    ) -> Result<EditResult>
    {
        ...
        for file_block in &spec.file_blocks
        {
            if !file_block
                ...
                .exists()
            {
                conflicts.push(EditConflict::FileNotFound(
                    file_block
                        .path
                        .clone(),
                ));
        ...
        {
            return Ok(EditResult { applied_files, conflicts, backup_paths });
        }
        ...
        Ok(EditResult { applied_files, conflicts, backup_paths })
    }
    ...
    ) -> Result<(), EditConflict>
    {
        match op
        {
            EditOperation::Replace { start_line, end_line, old_content, guard_cid, .. } =>
            {
                // Check span bounds
                if *start_line == 0
                    || *end_line == 0
    ...
    ) -> Result<()>
    {
        ...
        sorted_ops.sort_by(|a, b| {
            let key = |op: &EditOperation| -> (usize, u8, usize) {
                match op
                {
                    EditOperation::Delete { start_line, end_line } => (*start_line, 0, *end_line),
                    EditOperation::Replace { start_line, end_line, .. } =>
                    {
        ...
        for op in sorted_ops
        {
            match op
            {
                EditOperation::Replace {
                    start_line, end_line, old_content, new_content, ..
                } =>
                {
...
) -> Result<()>
{
        ...
        .is_empty()
    {
        ...
        // If --resolve flag is set and we're applying (not just previewing), try to resolve
        // conflicts
        if args.resolve && run_mode == RunMode::Apply
        {
                ...
                && !args.force
            {
                return Err(ApplyCliError::Conflicts(format!(
                    "{} conflicts remain after auto-resolution. Use --force to apply despite \
                     conflicts.",
                    updated_preview
                        .conflicts
                        .len()
        ...
        {
            return Err(ApplyCliError::Conflicts(format!(
                "{} conflicts detected. Use --resolve for auto-resolution or --force to apply \
                 despite conflicts.",
                preview
                    .conflicts
                    .len()
...
impl TokenMatcher
{
    ...
    ) -> Option<(usize, usize)>
    {
        ...
        let hay_ids = self
            .bpe
            .encode_ordinary(&normalize_for_cid(&hay));
        let nee_ids = self
            .bpe
            .encode_ordinary(&norm);
...
{
    let resp = show_session(repo_root, ShowRequest {
        id: a
            .id
            .clone(),
        verbose: a.verbose,
...
"#;
        ...
        match op
        {
            EditOperation::Replace { old_content, new_content, .. } =>
            {
